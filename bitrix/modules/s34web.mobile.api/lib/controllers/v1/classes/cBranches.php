...
class cBranches
{
    public const CACHE_ID = "branches";
  ...
  /**
    ...
    public static function getBranches()
    {
        $result = new ResultData();
        $action = function () {
            return self::getBranchesList();
        };
        $data = self::cachedataD7('branches', $action, 'branches_list', 604800, cGeneral::getIBlockIDByLang("branches"));
        ...
        try {
            if (Loader::includeModule("iblock")) {
                ...
                foreach ($branches as $branch) {
                    $branch['PHONES']['VALUE'] = gettype($branch['PHONES']['VALUE']) === 'boolean' ? [] : $branch['PHONES']['VALUE'];
                    $branch = [
                        'id' => intval($branch['ID']),
                        'name' => $branch['NAME'],
                        ...
                        'picture' => CFile::GetPath($branch['DETAIL_PICTURE']),
                    ];
                    $result[] = $branch;
                }
            } else {
