...
class cProducts
{
    public const CACHE_ID = "products";
  ...
  /**
    ...
    public static function getProducts()
    {
        ...
        $data = self::cachedataD7('products', $action, 'products_list', 604800, cGeneral::getIBlockIDByLang("products"));
    ...
    /**
     * Метод для получения услуг автосервиса по id
     * @param int $branch_id
     * @return ResultData
     */
    public static function getBranchProducts(int $branch_id) {
        $result = new ResultData();
        $products = self::getBranchProductsList($branch_id);
        $data = $products;
        if (!$data['error'] && !$data['error_message']) {
            $result->setData($data['result']);
        } else {
...
//                    'LENGTH',
    ...
    */
    ...
    private static function getBranchProductsList($branch_id)
    {
        $error = false;
        $error_message = false;
        $result = [];
        try {
            if (Loader::includeModule("iblock"))
            {
                $arFilter = [
                    'IBLOCK_ID' => 4,
                    'PROPERTY_BRANCHES.ID' => $branch_id,
                ];
                $products = CIBlockElement::GetList(false, $arFilter);
                while ($product = $products->GetNextElement()) {
                    $product = array_merge($product->GetFields(), $product->GetProperties());
                    $product = [
                        'id' => intval($product['ID']),
                        'name' => $product['NAME'],
                        'working_hour' => $product['WORKING_HOUR']['VALUE'],
