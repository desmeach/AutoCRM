...
class cCatalog
{
    public const CACHE_ID = "catalog";
    //const CATALOG_IBLOCK_ID = 39;
  ...
  /**
    ...
    public static function getCategoriesMain()
    {
        ...
        $data = self::cachedataD7('catalogmain', $action, 'catalogmain_sections', 604800, cGeneral::getIBlockIDByLang("catalog"));
     ...
     * @return ResultData
     */
    public static function getCategories($category_id = 0)
    {
        $result = new ResultData();
        if (empty($category_id))
            $category_id = 0;
        $action = function () use ($category_id) {
            return self::getItemsSections(cGeneral::getIBlockIDByLang("catalog"), $category_id);
        };
        $data = self::cachedataD7('catalog', $action, 'catalog_sections' . $category_id, 604800, cGeneral::getIBlockIDByLang("catalog"));
     ...
     */
    public static function getExcludedProperties()
    {
        static $exclude_props = [];

        try {
            if ($exclude_props === []) {
                $properties = Option::get('s34web.mobile.api', 'catalog_excluded_properties', '');

                if (empty($properties)) {
                    $exclude_props = self::EXCLUDED_PROPERTIES;
                    Option::set(
                        's34web.mobile.api',
                    ...
                    $exclude_props = unserialize($properties);
                }
            }
        } catch (\Exception $e) {
            cGeneral::dump('Ошибка при получение списка исключенных свойств: '.$e->getMessage(), '_exception');
        } finally {
...
//                    'LENGTH',
    ...
    */
    ...
    private static function getItemsSections($iblock_id, $parent_section_id, $depth_level=0)
    {
        $error = false;
        $error_message = false;
        $result = [];
