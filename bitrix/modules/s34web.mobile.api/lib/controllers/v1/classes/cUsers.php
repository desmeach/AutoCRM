...
class cUsers
{
  ...
  /**
     ...
     */
    public static function setUserData($userPhoneFrom, $userPhoneTo, $car) {
        $result = new ResultData();
        $data = [];
        if (strlen($name) > 0) {
    ...
    public static function transferCar($userPhoneFrom, $userPhoneTo, $car) {
        $result = new ResultData();
        $data = [];

        $userFrom = ClientsController::getByPhone($userPhoneFrom);
        $userTo = ClientsController::getByPhone($userPhoneTo);

        $userFromCars = [];
        foreach ($userFrom['CARS']['VALUE'] as $userCar) {
            if ($userCar['ID'] !== $car)
                $userFromCars[] = $userCar['ID'];
        }
        if (empty($userFromCars))
            $userFromCars = "";
        CIBlockElement::SetPropertyValuesEx($userFrom['ID'], 1, ['CARS' => $userFromCars]);

        $userToCars = [];
        foreach ($userTo['CARS']['VALUE'] as $userCar) {
            $userToCars[] = $userCar['ID'];
        }
        $userToCars[] = $car;
        CIBlockElement::SetPropertyValuesEx($userTo['ID'], 1, ['CARS' => $userToCars]);

        $userTo = ClientsController::getByID($userTo['ID']);
        $key = empty($userTo['KEY']['VALUE']) ? "" : $userTo['KEY']['VALUE'];
        CIBlockElement::SetPropertyValuesEx($car, 2, ['KEY' => $key]);
