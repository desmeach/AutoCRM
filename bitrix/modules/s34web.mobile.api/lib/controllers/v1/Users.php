...
use s34web\Mobile\Api\controllers\v1\classes\cBranches;
...
use s34web\Mobile\Api\controllers\v1\classes\cUsers;
...
class Users {
    ...
    public function getBranches()
    {
        ...
        $arResult = cBranches::getBranches();
    ...
    public function userData()
    {
        ...
        if (cGeneral::checkMethod($arRequest, "GET")) {
            ...
            $userPhone = $arRequest["parameters"]["phone"];
            $arResult = cUsers::getUserData($userPhone);
            cGeneral::sendResponse($arResult, $arRequest);
        } elseif (cGeneral::checkMethod($arRequest, "POST")) {
            $name = $arRequest["parameters"]["name"];
            $email = $arRequest["parameters"]["email"];
            $phone = $arRequest["parameters"]["phone"];

            $arResult = cUsers::setUserData(
                $name,
                $phone,
                $email,
            );
    ...
    public function transferCar() {
        ...
        if (cGeneral::checkMethod($arRequest, "PUT")) {
            $userPhoneFrom = $arRequest["parameters"]["phone_from"];
            $userPhoneTo = $arRequest["parameters"]["phone_to"];
            $car = $arRequest["parameters"]["car"];
            $arResult = cUsers::transferCar($userPhoneFrom, $userPhoneTo, $car);
            cGeneral::sendResponse($arResult, $arRequest);
        } else {
            $arTmp = new ResultData();
            $arTmp->setErrors(cErrors::INCORRECT_METHOD);
